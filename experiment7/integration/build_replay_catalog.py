...
def _manifest_metadata(root: Path) -> dict[int, dict[str, str]]:
...
def _episode_id(path: Path, payload: dict[str, Any]) -> int:
    try:
...
def _extract_decks(payload: dict[str, Any], source: Path) -> list[tuple[int, ...]]:
...
def build_catalog(
    ...
    metadata = _manifest_metadata(raw_root)
    ...
    for index, path in enumerate(paths, start=1):
        ...
        try:
            ...
            timestamp = parse_timestamp(create_time, path.stat().st_mtime)
