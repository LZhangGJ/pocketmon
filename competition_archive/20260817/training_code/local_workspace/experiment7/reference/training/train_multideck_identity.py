...
def main() -> None:
    ...
    pretrain_weights = np.asarray(
        [
            ...
            * MODULE_WEIGHTS[
                module_by_episode[int(pretrain.sequence.base.data["episode_ids"][decision])]
            ]
    ...
    if np.any(class_counts == 0):
        raise RuntimeError(f"fit split has empty opponent classes: {class_counts.tolist()}")
    class_weights_np = 1.0 / np.sqrt(class_counts.astype(np.float64))
