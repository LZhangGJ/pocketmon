...
def collate_rows(rows: list[dict[str, Any]], device: torch.device) -> dict[str, Any]:
    ...
    option_logit_bias = np.zeros((batch, max_options), dtype=np.float32)
    for index, row in enumerate(rows):
        ...
        if count:
            ...
            for masked in row.get("hard_masked_options", ()):
                masked_index = int(masked)
                if not 0 <= masked_index < count:
                    raise Experiment7Error(
                        f"hard-masked option index out of range: {masked_index}/{count}"
                    )
                option_mask[index, masked_index] = 0
            biases = row.get("option_logit_bias")
            if biases is not None:
                if len(biases) != count:
                    raise Experiment7Error(
                        f"option-logit bias length mismatch: {len(biases)}/{count}"
                    )
                option_logit_bias[index, :count] = np.asarray(
                    biases, dtype=np.float32
                )
    ...
    return {
        ...
        "option_logit_bias": tensor(option_logit_bias),
...
def evaluate_actions(
    ...
    for step in range(int(lengths.max().item()) + 1):
        ...
        logits[:, :option_count] = (
            logits[:, :option_count] + batch["option_logit_bias"]
        )
...
def sample_action(
    ...
    for tokens in range(int(batch["max_count"][0].item()) + 1):
        ...
        logits[:option_count] = logits[:option_count] + batch["option_logit_bias"][0]
...
def ppo_loss(
    ...
    *,
    ...
    tactical_preference_coefficient: float = 0.04,
    ...
    preference_indices = [
        ...
        if row.get("tactical_preferred_action")
        and list(row["tactical_preferred_action"]) != list(row["action"])
    ]
    tactical_preference = values.new_zeros(())
    if preference_indices:
        preferred_rows = [
            {**rows[index], "action": list(rows[index]["tactical_preferred_action"])}
            for index in preference_indices
        ]
        preferred_log_probability, _, _ = evaluate_actions(
            model, collate_rows(preferred_rows, values.device)
        )
        chosen_for_preference = new_log_probability[
            torch.tensor(preference_indices, dtype=torch.long, device=values.device)
        ]
        tactical_preference = torch.nn.functional.softplus(
            chosen_for_preference - preferred_log_probability
        ).mean()
    if was_training:
        model.train()
    ...
    sample_weights = torch.tensor(
        [
            (seat1_weight if int(row.get("player", 0)) == 1 else 1.0)
    ...
    loss = (
        ...
        + tactical_preference_coefficient * tactical_preference
    ...
    return loss, {
        ...
        "tacticalPreferenceLoss": float(tactical_preference.detach()),
        "tacticalPreferenceRows": float(len(preference_indices)),
