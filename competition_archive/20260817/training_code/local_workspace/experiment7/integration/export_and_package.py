...
def add_runtime_diagnostics(main_path: Path) -> None:
    """Insert diagnostics before agent so Kaggle still selects agent as entrypoint."""
    source = main_path.read_text(encoding="utf-8")
    marker = "\ndef agent("
    offset = source.find(marker)
    if offset < 0:
        raise Experiment7Error(f"runtime main has no top-level agent function: {main_path}")
    diagnostics = (
        "\n\ndef diagnostics():\n"
        "    return {\"bc\": bc_advisor.get_stats()}\n"
    )
    main_path.write_text(
        source[:offset] + diagnostics + source[offset:],
        encoding="utf-8",
    )
...
def package_universal_agents(
    ...
    def link_or_copy(source: Path, target: Path) -> None:
        """Deduplicate immutable package assets when source/output share a filesystem."""
        try:
    ...
    for row in selected:
        ...
        add_runtime_diagnostics(package / "main.py")
        ...
        link_or_copy(portable, package / "deck_identity_bc.npz")
        link_or_copy(engine_catalog, package / "engine_catalog.json")
...
def package_agents(reference_root: Path, sources_path: Path, portable: Path, output_root: Path) -> dict[str, Any]:
    ...
    for row in manifest["currentSources"]:
        ...
        add_runtime_diagnostics(main_path)
        shutil.copy2(runtime / "portable.py", package / "portable.py")
