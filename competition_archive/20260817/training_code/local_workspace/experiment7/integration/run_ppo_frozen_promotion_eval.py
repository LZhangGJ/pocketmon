...
def is_key_matchup(
    ...
    *,
    ...
    if chain not in key_chains:
        return False
    searchable = " ".join(
        str(opponent.get(field, ""))
        for field in (
            "name",
            "archetype",
            "canonical_archetype",
...
def ensure_bc_packages(
    *, eval_root: Path, league: dict[str, Any], worktree: Path, python: Path, portable: Path
    ...
    expected_decks = {chain["deckSha256"] for chain in league["chains"].values()}
    if manifest.is_file():
        cached = load(manifest)
        cached_decks = {row.get("deckSha256") for row in cached.get("packages", [])}
        if expected_decks <= cached_decks:
            return cached
        # Keep the previous baseline intact: it may still be referenced by an
        # older completed round.  Build a new cache whenever league decks grow.
        baseline = eval_root / f"universal-bc-baseline-{len(expected_decks)}decks"
        manifest = baseline / "packages" / "packages.json"
        if manifest.is_file():
            cached = load(manifest)
            cached_decks = {row.get("deckSha256") for row in cached.get("packages", [])}
            if expected_decks <= cached_decks:
                return cached
            baseline = eval_root / (
                f"universal-bc-baseline-{len(expected_decks)}decks-"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run an immutable PPO frozen-promotion matrix")
    ...
    parser.add_argument("--games-per-key-frozen", type=int, default=40)
    parser.add_argument(
        "--exclude-pool-status",
        action="append",
        default=[],
        help="exclude training-only pool rows from promotion aggregates",
    )
    parser.add_argument(
        "--key-chains",
        default="a02_grim_large_g9,a02_grim_large_g9_pokegear",
    )
    parser.add_argument(
        "--key-opponent-patterns",
        default="alakazam,ogerpon,crustle,grim,dragapult,festival",
    )
    ...
    key_chains = csv_values(args.key_chains)
    key_opponent_patterns = csv_values(args.key_opponent_patterns)
    if args.games_per_key_frozen < args.games_per_frozen:
        raise ValueError("games per key frozen must be >= broad frozen games")
    if args.games_per_key_frozen % 2:
        raise ValueError("games per key frozen must be even for paired seats")

    league_root = args.league_root.resolve()
    eval_root = league_root / "monitoring" / "ppo-frozen-promotion"
    ...
    promotion_state = load(args.promotion_state.resolve())
    base_pool = load(Path(league["basePool"]["path"]))
    excluded_pool_status = set(args.exclude_pool_status)
    frozen = [
        row
        for row in base_pool.get("agents", [])
        if row.get("status", "accepted") == "accepted"
        and str(row.get("pool_status", "")) not in excluded_pool_status
    ]
    if not frozen:
    ...
    for chain_name in promotion_state.get("policy", {}).get("chains", []):
        chain = league["chains"].get(chain_name)
        candidate = promotion_state.get("candidates", {}).get(chain_name)
        if not chain or not candidate or candidate.get("status") != "testing":
            continue
        manifest_path = candidate.get("packageManifest")
        if not manifest_path:
            raise RuntimeError(f"candidate has no deployed package: {chain_name}")
        ...
        selected.append(
            {
                ...
                "deckSha256": candidate["deckSha256"],
                "archetypeId": candidate["archetypeId"],
            }
        )
    if not selected:
        print(json.dumps({"status": "idle", "reason": "no_testing_candidates"}))
        return 0
    ...
    metadata = {
        ...
        "excludedPoolStatus": sorted(excluded_pool_status),
        "gamesPerFrozen": args.games_per_frozen,
        "gamesPerKeyFrozen": args.games_per_key_frozen,
        "keyChains": sorted(key_chains),
        "keyOpponentPatterns": sorted(key_opponent_patterns),
    ...
    for chain_index, selected_row in enumerate(selected):
        ...
        for opponent_index, opponent in enumerate(frozen):
            ...
            games = (
                args.games_per_key_frozen
                if is_key_matchup(
                    selected_row["chain"],
                    opponent,
                    key_chains=key_chains,
                    patterns=key_opponent_patterns,
                )
                else args.games_per_frozen
            )
            paired_games(schedule, ppo_name, opponent["name"], games, pair_seed)
            paired_games(schedule, bc_name, opponent["name"], games, pair_seed)
        paired_games(
            schedule,
            ppo_name,
            bc_name,
    ...
    for selected_row in selected:
        ...
        key_agent_names = {row["agent"] for row in agent_rows if row["keyMatchup"]}
        ppo_key_rows = [row for row in ppo_frozen_rows if row["opponent"] in key_agent_names]
        bc_key_rows = [row for row in bc_frozen_rows if row["opponent"] in key_agent_names]
        key_aggregate = metrics(ppo_key_rows)
        bc_key_aggregate = metrics(bc_key_rows)
        previous_aggregate = (
            previous.get("chains", {}).get(chain, {}).get("frozenAggregate")
            if previous_compatible
            else None
        )
        previous_key_aggregate = (
            previous.get("chains", {}).get(chain, {}).get("keyFrozenAggregate")
            if previous_compatible
            else None
        )
        delta_previous = (
        ...
        if delta_previous is None:
            ...
            progress = "flat"
        key_delta_previous = (
            key_aggregate["scoreRate"] - previous_key_aggregate["scoreRate"]
            if previous_key_aggregate
            and key_aggregate["scoreRate"] is not None
            and previous_key_aggregate.get("scoreRate") is not None
            else None
        )
        ...
        report[chain] = {
            ...
            "keyFrozenAggregate": key_aggregate,
            "universalBcKeyFrozenAggregate": bc_key_aggregate,
            "keyOpponentCount": len(key_agent_names),
            "keyGamesPerOpponent": args.games_per_key_frozen if key_agent_names else 0,
            "keyDeltaVsPrevious": key_delta_previous,
            "keySeatMetrics": {
                seat: metrics([row for row in ppo_key_rows if row["learner_seat"] == seat])
                for seat in ("0", "1")
            },
