...
SOURCE_REF = "origin/agent/experiment7-team-code-20260808"
SOURCE_PREFIX = "experiment7/source/experiment7_code_for_gpt_2026-08-08.zip.b64.part-"
...
def main() -> None:
    ...
    subprocess.run(
        ["git", "fetch", "origin", "agent/experiment7-team-code-20260808"],
        cwd=repo,
        check=True,
    )
    names = git_output(
    ...
    parts = sorted(name for name in names if name.startswith(SOURCE_PREFIX))
    if not parts:
        raise RuntimeError(f"no source chunks found at {SOURCE_REF}:{SOURCE_PREFIX}*")
    encoded = b"".join(git_output(repo, "show", f"{SOURCE_REF}:{name}") for name in parts)
    ...
    with tempfile.TemporaryDirectory() as temporary_text:
        ...
        rows = list(
            csv.DictReader(
                (extracted / "PACKAGE_MANIFEST.csv").open(
