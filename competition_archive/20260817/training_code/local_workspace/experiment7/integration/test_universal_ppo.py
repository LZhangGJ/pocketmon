...
from ppo_tactical_shaping import (  # noqa: E402
...
class UniversalPpoTest(unittest.TestCase):
    ...
    def test_hard_mask_is_applied_to_sampling_and_log_probability(self) -> None:
        prepared = row()
        prepared["hard_masked_options"] = [1, 2]
        action, log_probability, value, entropy = sample_action(
            self.model.eval(), prepared, self.device
        )
    ...
    def test_a08_end_only_and_gated_modes_still_penalize_end(self) -> None:
        observation = {
            "current": {"yourIndex": 0, "players": [{"active": [], "bench": []}, {}]},
            "select": {},
        }
        options = [{"type": 9, "inPlayArea": 4}, {"type": 13}, {"type": 14}]
        features = type(
            "Features",
            (),
            {"resolve_option_cards": staticmethod(lambda *_: (None, None))},
        )
        for mode in ("end_only", "gated"):
            with self.subTest(mode=mode):
                ...
                self.assertEqual(adjustment.preferred_action, (0,))
                self.assertTrue(adjustment.evolve_target_active)

    def test_a08_gated_attack_penalizes_only_non_scoring_nonterminal_attack(self) -> None:
        observation = {
            "current": {"yourIndex": 0, "players": [{"active": [], "bench": []}, {}]},
            "select": {},
        }
        ...
        self.assertAlmostEqual(candidate.deferred_attack_penalty, 0.10)

        penalized = finalize_deferred_adjustment(
            candidate, ko=False, prize_delta=0, terminal_after_action=False
        )
        self.assertAlmostEqual(penalized.reward, -0.10)
        self.assertEqual(
            penalized.events,
            ("a08_nonlethal_attack_before_high_value_evolve",),
        )
        self.assertEqual(penalized.preferred_action, (0,))
        ...
        for outcome in (
            ...
            with self.subTest(outcome=outcome):
                exempt = finalize_deferred_adjustment(candidate, **outcome)
                self.assertEqual(exempt.reward, 0.0)
                self.assertEqual(exempt.events, ())
                self.assertEqual(exempt.preferred_action, ())

    def test_finish_trajectory_writes_a08_audit_fields(self) -> None:
        rows = [
            {
                "player": 0,
                "tactical_reward": 0.0,
        ...
        finish_trajectory(rows, 0)
        self.assertTrue(rows[0]["final_win"])
        self.assertFalse(rows[0]["ko"])
        self.assertEqual(rows[0]["prize_delta"], 0)
        self.assertTrue(rows[0]["evolve_target_active"])
        self.assertAlmostEqual(rows[0]["tactical_reward"], -0.10)
        self.assertAlmostEqual(rows[0]["reward"], 0.90)
        self.assertNotIn("_tactical_deferred_attack_penalty", rows[0])
