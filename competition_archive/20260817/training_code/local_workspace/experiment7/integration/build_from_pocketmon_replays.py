...
_ZIP_REPLAY_HANDLES: dict[Path, zipfile.ZipFile] = {}


def read_replay_bytes(locator: str) -> bytes:
    """Read a catalog replay path, including a member streamed from a ZIP."""
    if not locator.startswith("zip://"):
        return Path(locator).read_bytes()
    archive_text, member = locator[len("zip://") :].split("#", 1)
    archive_path = Path(archive_text).resolve()
    archive = _ZIP_REPLAY_HANDLES.get(archive_path)
    if archive is None:
        archive = zipfile.ZipFile(archive_path)
        _ZIP_REPLAY_HANDLES[archive_path] = archive
    return archive.read(member)


def close_replay_archives() -> None:
    for archive in _ZIP_REPLAY_HANDLES.values():
        archive.close()
    _ZIP_REPLAY_HANDLES.clear()
...
def write_tensordict_memmap_store(
    output_dir: Path, arrays: dict[str, np.ndarray]
) -> dict[str, Any]:
    """Write a dependency-free TensorDict-style memory-mapped tensor store.

    Each key is an independent ``.npy`` tensor so readers can use
    ``numpy.load(..., mmap_mode="r")``.  The directory is published atomically;
    an interrupted writer therefore cannot be mistaken for a complete cache.
    """
    ...
    try:
        ...
        write_json(temporary / "meta.json", metadata)
        ...
        raise
    metadata["path"] = str(output_dir.resolve())
    metadata["bytes"] = int(
        sum(path.stat().st_size for path in output_dir.glob("*.npy"))
    )
    return metadata
    ...
    *,
    ...
    with gzip.open(raw_path, "wt", encoding="utf-8", newline="\n", compresslevel=6) as raw_handle:
        ...
        for row_index, row in enumerate(rows, start=1):
            replay_locator = str(row["raw_path"])
            raw = read_replay_bytes(replay_locator)
    ...
    memmap_receipt = write_tensordict_memmap_store(
        output_dir / "features_tensordict", feature_arrays
    )
    ...
    summary = {
        ...
        "artifacts": {
            ...
            "features_tensordict": {
                "bytes": memmap_receipt["bytes"],
                "kind": memmap_receipt["kind"],
                "path": memmap_receipt["path"],
            },
