...
class PromotionGateTests(unittest.TestCase):
    ...
    def test_new_gate_does_not_retroactively_block_old_candidate(self) -> None:
        candidate = self.candidate()
        candidate["publishedAt"] = "2026-08-14T00:26:03+00:00"
        refresh_candidate_tactical_gate(
            candidate,
            {
                "enabled": True,
                "appliesToPublishedAtOrAfter": "2026-08-14T03:19:19+00:00",
                "minimumRevision": 11,
            },
        )
        self.assertFalse(candidate["tacticalGate"]["appliesToCandidate"])
    ...
    def test_key_matchup_gate_requires_forty_games_per_opponent(self) -> None:
        candidate = self.candidate()
        candidate["tacticalGate"] = {
            "appliesToCandidate": True,
            "minimumRevision": 11,
            ...
            "minimumTrackedOpportunities": 10,
            "maximumAggregateErrorRate": 0.35,
            "keyMatchupGate": {"enabled": True, "gamesPerOpponent": 40},
        }
        report = self.report("r1")
        report["chains"][candidate["chain"]].update(
            {
                "keyOpponentCount": 2,
                "keyFrozenAggregate": {"completed": 76, "failures": 0, "scoreRate": 0.5},
                "keyDeltaVsPrevious": 0.0,
                "agents": [],
            }
        )
        evidence = {
        ...
        self.assertFalse(observation["passed"])
        self.assertIn("insufficient_key_matchup_games", observation["reasons"])

    def test_key_matchup_gate_rejects_single_opponent_regression(self) -> None:
        candidate = self.candidate()
        candidate["tacticalGate"] = {
            "appliesToCandidate": True,
            "minimumRevision": 11,
        ...
        report["chains"][candidate["chain"]].update(
            {
                "keyOpponentCount": 1,
                "keyFrozenAggregate": {"completed": 40, "failures": 0, "scoreRate": 0.5},
                "keyDeltaVsPrevious": 0.0,
                "agents": [
                    {
                        "agent": "public_alakazam_search_v9",
                        "keyMatchup": True,
                        "ppo": {"completed": 40, "failures": 0},
                        "deltaVsPrevious": -0.11,
                    }
                ],
            }
        )
        ...
        self.assertFalse(observation["passed"])
        self.assertIn(
            "key_agent_regressed:public_alakazam_search_v9", observation["reasons"]
        )
