...
eval_league_root=/suedata1/Free/lzhang/pocketmon-runs/experiment7-hourly-image-whitelist-v4-20260815
hourly_root="$eval_league_root/monitoring/hourly-cache-pool"
state_dir="$eval_league_root/state"
driver="$eval_league_root/control/run_hourly_cache_pool_eval_v4_image_whitelist.py"
worktree=/homes/lzhang/worktrees/experiment7-async-4ppo-f8824b0
python=/homes/lzhang/mypath/new/envs/trans/bin/python3.11

if pgrep -af 'run_hourly_cache_pool_eval_v[34].*--league-root' >/dev/null; then
  echo "HOURLY_CACHE_POOL_EVAL_ALREADY_RUNNING"
  pgrep -af 'run_hourly_cache_pool_eval_v[34].*--league-root'
  exit 0
fi
...
log="$hourly_root/manual-$timestamp-image-whitelist-v4-d16.log"
nohup "$python" -s "$driver" \
  --league-root "$eval_league_root" \
  --worktree "$worktree" \
  --python "$python" \
  --run-shard "$eval_league_root/control/run_isolated_arena_shard_v4_retry.sh" \
...
echo "HOURLY_CACHE_POOL_IMAGE_WHITELIST_V4_STARTED pid=$pid"
echo "LOG $log"
