...
def resolve_direct_daily_slug(requested_date: str | None) -> tuple[str, str]:
    """Resolve a known public daily dataset without waiting for the lagging index."""
    if not requested_date:
        raise ValueError("--direct-date requires --date YYYY-MM-DD")
    return requested_date, DAILY_SLUG_TEMPLATE.format(date=requested_date)
...
def main() -> None:
    ...
    parser.add_argument(
        "--direct-date",
        action="store_true",
        help="Skip the index and use the official daily slug for --date after availability was verified",
    )
    ...
    direct_date = args.direct_date or os.getenv("PTCG_DIRECT_DATE") == "1"
    if direct_date:
        date, daily_slug = resolve_direct_daily_slug(args.date)
    else:
