...
def validate_kaggle_entrypoint_order(path: Path) -> None:
    """Reject agents that Kaggle would bind to a trailing helper callable."""
    ...
    if not definitions or definitions[-1] != "agent":
        trailing = definitions[-1] if definitions else "<none>"
        raise ValueError(
            "Kaggle selects the last top-level callable from main.py; "
            f"expected agent to be last, got {trailing}"
        )
...
def build(agent_dir: Path, cg_dir: Path, output: Path) -> Path:
    ...
    read_deck(deck_csv)
    validate_cg(cg_dir)
    validate_kaggle_entrypoint_order(main_py)
