...
def train_majority_mapping(df: pd.DataFrame) -> dict:
    ...
    for _, row in grouped.iterrows():
        key = "|".join(str(row[column]) for column in FEATURE_COLUMNS)
        if key not in mapping:
            mapping[key] = row["target_action"]
    return {
        "schema_version": 2,
        "label_alignment": "previous",
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train a diagnostic majority table from validated replay rows")
    ...
    model = train_majority_mapping(pd.read_csv(in_path))
