...
def main() -> None:
    ...
    with state_lock(lock_path):
        ...
        # Persistent experimental branches live outside the historical champion
        # registry. Preserve them if this installer is re-run later.
        current_base = read_json(Path(league["basePool"]["path"]).resolve())
        persistent_agents = [
            row
            for row in current_base.get("agents", [])
            if row.get("pool_status") == "persistent_ppo_branch"
        ]
        present = {row.get("name") for row in base_agents}
        base_agents.extend(row for row in persistent_agents if row.get("name") not in present)
        champions = []
