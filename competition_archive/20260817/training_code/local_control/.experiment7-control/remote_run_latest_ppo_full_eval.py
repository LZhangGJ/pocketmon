...
def main() -> int:
    ...
    # The cached baseline manifest predates chains added later (notably the
    # A02 Pokegear variant).  A completed prior matrix already contains the
    # verified same-deck BC packages, so merge those immutable package rows by
    # deck hash instead of rebuilding or overwriting the cache.
    prior_latest_path = eval_root / "latest.json"
    if prior_latest_path.is_file():
        prior_latest = load(prior_latest_path)
        prior_round_path = prior_latest.get("roundPath")
        if prior_round_path:
            prior_learners_path = Path(prior_round_path) / "learners.json"
            if prior_learners_path.is_file():
                prior_learners = load(prior_learners_path)
                bc_by_deck.update(
                    {
                        row["deckSha256"]: {
                            "name": row["name"],
