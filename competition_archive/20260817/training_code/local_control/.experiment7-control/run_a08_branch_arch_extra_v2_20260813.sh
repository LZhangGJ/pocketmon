...
extra_name=arch-extra-v2
extra_root="$root/$extra_name"
...
run_all() {
  mkdir -p "$extra_root/logs"
  "$python" -s /homes/lzhang/prepare_a08_branch_arch_extra_20260813.py \
    --branch-eval-root "$root" --extra-name "$extra_name" \
  ...
  for name in a08_maxbelt_g0020 a08_lilligant_g0020 a08_lilligant_maxbelt_g0015; do
    candidate="$extra_root/$name"
    echo "ARCH_EXTRA_V2_START candidate=$name at=$(date -Iseconds)"
    PYTHONNOUSERSITE=1 "$python" -s "$runner" \
      ...
      --games-per-frozen 40 --games-per-head-to-head 40 --shards 8 \
      --distributed-hosts "$hosts" --max-shards-per-host 1 \
      >"$extra_root/logs/$name.log" 2>&1
    echo "ARCH_EXTRA_V2_DONE candidate=$name at=$(date -Iseconds)"
  done
  touch "$extra_root/COMPLETE"
}

if [[ "${1:-}" == --run ]]; then run_all; exit; fi
mkdir -p "$extra_root"
pidfile="$extra_root/controller.pid"
if [[ -s "$pidfile" ]]; then
  old=$(<"$pidfile")
  if [[ "$old" =~ ^[0-9]+$ ]] && kill -0 "$old" 2>/dev/null; then
    echo "ARCH_EXTRA_V2_ALREADY_RUNNING pid=$old"
    exit
  fi
fi
nohup /bin/bash "$0" --run >"$extra_root/controller.log" 2>&1 </dev/null &
pid=$!
printf '%s\n' "$pid" >"$pidfile"
sleep 2
kill -0 "$pid"
echo "ARCH_EXTRA_V2_CONTROLLER_STARTED pid=$pid"
