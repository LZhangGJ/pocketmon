...
from rl.temporal_agent_adapter import TemporalRLBCPolicyAdapter
...
POLICY = TemporalRLBCPolicyAdapter(
    CHECKPOINT,
    fallback=_legal_fallback,
    device=os.environ.get("POCKETMON_RL_DEVICE", "cpu"),
    confidence_threshold=float(
        os.environ.get("POCKETMON_RL_CONFIDENCE", "0.0")
    ),
    deck=DECK,
)
...
def diagnostics() -> dict[str, Any]:
    ...
    result["temporal_specialist"] = True
    return result
